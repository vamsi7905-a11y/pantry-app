...
# Streamlit Admin App
# -----------------------
st.set_page_config(page_title="Pantry Admin", page_icon="📊", layout="wide")
st.title("📊 Pantry Admin Dashboard")

df = load_data()

if df.empty:
    st.warning("⚠️ No entries found yet.")
    st.stop()
...
col1.metric("Total Entries", len(filtered))
col2.metric("Unique Coupons", filtered["Coupon Number"].nunique())
col3.metric("Unique Items", filtered["Item"].nunique())

# -----------------------
# Pivot Summary
# -----------------------
st.subheader("📦 Items Issued / Returned")
...
st.download_button(
    label="📥 Download CSV",
    data=csv,
    file_name=f"pantry_summary_{datetime.now().strftime('%Y%m%d')}.csv",
    mime="text/csv",
)
